...
const PlanificacionEstudiante = () => {
    ...
    const [nuevoSprint, setNuevoSprint] = useState({ nombre: '', fechaInicio: '', fechaFin: '' });
    ...
    const editarHistoriaUsuario = (index) => {
        navigate(`/historia-usuario/${index}`, { state: { historia: historiasUsuario[index] } });
      };
    ...
    const guardarSprint = () => {
        if (nuevoSprint.nombre && nuevoSprint.fechaInicio && nuevoSprint.fechaFin) {
            if (isEditing) {
    ...
    const onDragStart = (e, historia) => e.dataTransfer.setData('historia', historia);
    ...
    return (
        <div className={`planificacion-estudiante ${isSidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
            ...
            <div className="contenido-con-sidebar">
                <aside className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''}`}>
                    <div className="sidebar-header">
                        ...
                        {!isSidebarCollapsed && (
                            <div className="project-info">
                                <h3>Sistema de Evaluación Basada en Proyecto</h3>
                            </div>
                        )}
                    </div>
                    ...
                    <ul className="sidebar-menu">
                        ...
                        <li className="menu-item">
                            <i className="fas fa-tag icon-menu"></i>
                ...
                <div className="contenido-principal">
                    <div className="contenedor-titulo-planificacion">
                        <h1 className="titulo-planificacion">Product Backlog</h1>
                        <hr className="divisor-planificacion" />
                    </div>

                    <div className="contenedor-principal">
                        <div className="contenedor-requerimientos">
                            <h2 className="titulo-requerimientos">Requerimientos</h2>
                            <div className="lista-requerimientos">
                                {requerimientos.map((req, index) => (
                                    <div key={index} className="item-requerimiento">
                                        ...
                                        <div className="iconos-acciones">
                                            ...
                                            <i
                                                ...
                                                onClick={() => removeItem(requerimientos, setRequerimientos, index)}
                                            ></i>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="contenedor-historias">
                            <h2 className="titulo-historias">Historias de Usuario</h2>
                            {historiasUsuario.length === 0 ? (
                                <p className="sprint-mensaje-vacio">
                                    No hay Historias de Usuario. Añade una para empezar.
                                </p>
                            ) : (
    ...
    {historiasUsuario.map((historia, index) => (
        <div
            ...
            draggable
            onDragStart={(e) => onDragStart(e, historia.titulo)}
        >
            <span>{historia.titulo}</span>
            <div className="iconos-acciones">
                <i
                    className="fas fa-edit icono-editar"
                    onClick={() => editarHistoriaUsuario(index)}
                ></i>
                <i
                    className="fas fa-trash-alt icono-eliminar"
                    onClick={() => removeItem(historiasUsuario, setHistoriasUsuario, index)}
                ></i>
            </div>
        </div>
    ))}
                            ...
                            <button className="boton-agregar" onClick={abrirModalHU}>
                                + Historia de usuario
                            </button>
                        </div>
                        ...
                        <div className="contenedor-sprints">
                            ...
                            {sprints.length === 0 ? (
                                <p className="sprint-mensaje-vacio">No hay sprints. Añade uno para empezar.</p>
                            ) : (
                                    ...
                                    {sprints.map((sprint, index) => (
                                        ...
                                        >
                                            <div className="sprint-header">
                                                <h3 className="sprint-titulo">{sprint.nombre}</h3>
                                                <div className="iconos-acciones">
                                                    <i
                                                        className="fas fa-edit icono-editar"
                                            ...
                                            <div className="sprint-contenido">
                                                {sprint.historias.length > 0 ? (
                                                    sprint.historias.map((historia, i) => (
                                                        <div key={i} className="item-historia-sprint">
                                                            <span>{historia}</span>
                                                            <i
                                                                className="fas fa-trash-alt icono-eliminar"
                                                                onClick={() => removeHistoriaFromSprint(index, i)}
                                                            ></i>
                                                        </div>
                                                    ))
                                                ) : (
            ...
            {isReqModalOpen && (
                <div className="sprint-modal-overlay">
                    <div className="sprint-modal">
                        ...
                        />
                        <div className="modale-buttons">
                            <button onClick={guardarRequerimiento} className="modale-button modale-button-guardar">
                                Guardar
                            </button>
                            <button onClick={cerrarModalReq} className="modale-button modale-button-cancelar">
                                Cancelar
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="hu-modal">
                        ...
                        <div className="adjuntos-container">
                            <div className="adjuntos-info">
                                ...
                                <button className="boton-adjuntar" onClick={() => fileInputRef.current.click()}>
                            ...
                            <div
                                className={`drag-drop-zone ${archivos.length > 0 ? 'with-files' : ''}`}
                                onDrop={handleDrop}
                                onDragOver={(e) => e.preventDefault()}
                            >
                                {archivos.length === 0 && <p className="drag-drop-text">¡Arrastre los archivos adjuntos aquí!</p>}

                                <div className="preview-container">
                                    {archivos.map((file, index) => (
                                        <div key={index} className="preview-item">
                                            <img src={file.preview} alt={`preview-${index}`} />
                                            <button className="boton-eliminar" onClick={() => eliminarArchivo(index)}>
                        ...
                        <div className="hu-modal-buttons">
                            <button className="hu-modal-button hu-modal-button-cancelar" onClick={cerrarModalHU}>
                                Cancelar
                            </button>
                            <button className="hu-modal-button hu-modal-button-crear" onClick={guardarHU}>
                                Crear
                            </button>
                        </div>
                    </div>
